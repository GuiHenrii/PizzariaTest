...
const pool = mysql.createPool({
    ...
    user: process.env.DB_USER || 'root',
    password: process.env.DB_PASS || '',
    database: process.env.DB_NAME || 'pizza_express_bot',
    port: process.env.DB_PORT || 3306,
    waitForConnections: true,
    connectionLimit: 30,
    queueLimit: 0
...
async function initDB() {
    try {
        ...
        await poolWithoutDB.query(`CREATE DATABASE IF NOT EXISTS \`${process.env.DB_NAME || 'pizza_express_bot'}\``);
        ...
        // Execute Seed
        const seedPath = path.join(__dirname, '../database', 'seed_pizza.sql');
