...
client.on('message', async msg => {
    ...
    if (result.isOrderCompleted && result.orderData) {
        ...
        const orderResult = await orderService.processNewOrder(msg.from, result.orderData);

        if (!orderResult.success) {
            await client.sendMessage(msg.from, "⚠️ Houve uma falha interna ao salvar seu pedido. Por favor, avise um atendente.");
        } else if (orderResult.isPix && orderResult.pixData) {
            // MODO PIX AUTOMÁTICO
            await client.sendMessage(msg.from, `🥩🔥 *Pedido #${orderResult.pedidoId} Registrado!*

Pague via Pix agora mesmo lendo o *QR Code* abaixo ou usando o *Pix Copia e Cola* no seu banco.

Assim que confirmarmos o pagamento (em questão de segundos), a nota sairá na cozinha e te avisaremos aqui, Não e necessário enviar o comprovante!`);

            // Envia o QRCode imagem
            if (orderResult.pixData.qr_code_base64) {
                const media = new MessageMedia('image/png', orderResult.pixData.qr_code_base64);
                await client.sendMessage(msg.from, media);
            }

            // Envia o Copia e Cola separadamente para ficar fácil de copiar
            await client.sendMessage(msg.from, '💳 *Copia e Cola:*');
            await client.sendMessage(msg.from, orderResult.pixData.qr_code);
        } else {
            // Pagamento na Entrega/Retirada (Dinheiro/Cartão)
            await client.sendMessage(msg.from, `🥩🔥 *Pedido #${orderResult.pedidoId} Confirmado!*
            
Sua comanda já foi enviada para nossa cozinha e logo entrará em preparo! Geralmente nossa entrega/preparo leva *em média de 30 a 40 minutos*.

Obrigado por escolher a Léo Churrascaria!`);
        }
    }
});
