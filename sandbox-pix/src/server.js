...
const whatsappConfig = require('./services/whatsapp.service');
...
app.post('/webhook/mercadopago', async (req, res) => {
    try {
        ...
        console.log(`[DEBUG WEBHOOK] MP enviou algo! Topic: ${topic}, ID: ${paymentId}`);

        const topicosValidos = ['payment', 'payment.updated', 'payment.created'];
        if (topicosValidos.includes(topic) && paymentId) {
            console.log(`[WEBHOOK MP] Recebida notificação de pagamento: ID ${paymentId}`);
            
            const paymentInfoStatus = await paymentService.consultarStatus(paymentId);
            console.log(`[PIX] Pagamento ID ${paymentId} - Status atual: ${paymentInfoStatus}`);
            
            if (paymentInfoStatus === 'approved') {
                // Tenta liberar o pedido (se impresso=2) e pega o ID
                const pedidoId = await orderService.liberarImpressaoPix(paymentId);
                
                if (pedidoId) {
                    console.log(`[PIX APROVADO] Pedido #${pedidoId} liberado para impressão!`);
                    
                    // Notifica o cliente via WhatsApp
                    const [clienteResult] = await db.pool.query('SELECT cliente_fone FROM pedidos WHERE id = ?', [pedidoId]);
                    if (clienteResult.length > 0) {
                        const phone = clienteResult[0].cliente_fone;
                        if (whatsappConfig.client) {
                            try {
                                await whatsappConfig.client.sendMessage(phone, `✅ *Uhu! Seu Pix Mágico acabou de cair!* 🎉\n\nRecebemos o pagamento do pedido #${pedidoId}. Ele acaba de ser impresso lá na cozinha!\n\nGeralmente nossa entrega/preparo leva *em média de 30 a 40 minutos*. Daqui a pouco o motoboy bate aí! 🥩🛵`);
                            } catch (e) {
                                console.error("Falha ao enviar aviso de Pix via Whats:", e);
                            }
                        }
                    }
                }
